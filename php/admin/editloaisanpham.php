...
if(!isset($_SESSION['Sdt'])){
    ...
    if (isset($_POST['submit'])){
        ...
        if (isset($_POST['id'])){
            $Layid = $_POST['id'];
            $sql=mysqli_query($connect,"UPDATE `loaisp` SET `Tenloai` = '$tenloai' WHERE `loaisp`.`idLoai` = $Layid;");
        }
        else {
            $sql=mysqli_query($connect,"INSERT INTO `loaisp` (`Tenloai`) VALUES ('$tenloai');");
        }
    ...
    if ($Them=="1"){
        ...
        $id = $_GET['id'];
        $layloai = mysqli_query($connect,"select * from loaisp where idLoai = $id; ");
...
<html>
    ...
    <form method="POST" action="../TrangChu.php"><button name="btnThoat"> (Đăng xuất)</button></form>
        ...
        <div>
            <a href="admin.php?menu=2" style="font-size: 24px;">[Quay về trang quản lý]</a>
        </div>
    <form  method="POST" action="editloaisanpham.php?Them=<?php echo $Them ?>&Sua=<?php echo $Sua ?><?php if ($Sua=="1"){echo "&id=".$id;} ?>">
        ...
        <p style="font-size:24px">Tên loại sản phẩm: </p>
        <input maxlength="100" type="text"  name="tenloai" style="width:200px;height:30px;margin:20px;padding:10px;border:1px solid" <?php if ($Sua=="1") {echo "value=\"".$layloai['Tenloai']."\"";} ?>/>
        <?php if ($Sua=="1"){ ?>
