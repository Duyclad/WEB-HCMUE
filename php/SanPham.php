...
if ($SX=="1"){
    $sql_sptnc = "select * from sanpham where Loaisp='$idLoai' order by Giasp  desc limit " . $item_per_page . " offset " . $offset;
    $querytnc = mysqli_query($connect, $sql_sptnc);
    $totalsp =  mysqli_query($connect, "select * from sanpham where Loaisp='$idLoai'");
$totalsp = $totalsp->num_rows;

$totalpage = ceil($totalsp / $item_per_page);
}
else if($SX=="2"){
    $sql_sptnc = "select * from sanpham where Loaisp='$idLoai' order by Giasp asc limit " . $item_per_page . " offset " . $offset;
    $querytnc = mysqli_query($connect, $sql_sptnc);
    $totalsp =  mysqli_query($connect, "select * from sanpham where Loaisp='$idLoai'");
$totalsp = $totalsp->num_rows;
...
$totalpage = ceil($totalsp / $item_per_page);
    ...
    $sql_sptnc = "select * from sanpham where Loaisp='$idLoai' order by id desc limit " . $item_per_page . " offset " . $offset;
    $querytnc = mysqli_query($connect, $sql_sptnc);
    $totalsp =  mysqli_query($connect, "select * from sanpham where Loaisp='$idLoai'");
$totalsp = $totalsp->num_rows;
...
<body>
    ...
    <hr>
    <h1 style="text-align: center;margin: 10px;"><?php echo $ploai['Tenloai'] ?></h1>
    <p style="font-size: 24px;">Sắp xếp theo: <a href="<?php echo "sanpham.php?idLoai=".$idLoai."&SX=2" ?>" style="font-size: 24px;<?php if ($SX=="2") {echo "font-weight: bold;";}; ?>">Giá tăng dần</a> - <a href="<?php echo "sanpham.php?idLoai=".$idLoai."&SX=1" ?>" style="font-size: 24px;<?php if ($SX=="1") {echo "font-weight: bold;";}; ?>">Giá giảm dần</a></p>
    ...
    <div style="margin: 20px;text-align:center">
        <?php
        if ($tranghientai > 3) {
            echo "<a href=\"?per_page=" . $item_per_page ."&SX=".$SX."&idLoai=".$idLoai . "&page=1\" style=\"border: 1px solid;margin:5px;font-size: 24px;padding:10px\">Trang đầu</a>";
        }
        if ($tranghientai > 1) {
            echo "<a href=\"?per_page=" . $item_per_page ."&SX=".$SX."&idLoai=".$idLoai ."&page= " . ($tranghientai - 1) . "\" style=\"border: 1px solid;margin:5px;font-size: 24px;padding:10px\">Trang trước</a>";
        }
        ...
        for ($i = 1; $i <= $totalpage; $i++) {
            ...
            if ($i != $tranghientai) {
                if ($i > $tranghientai - 3 && $i < $tranghientai + 3) {
                    echo "<a href=\"?per_page=" . $item_per_page ."&SX=".$SX."&idLoai=".$idLoai . "&page=" . $i . "\" style=\"border: 1px solid;margin:5px;font-size: 24px;padding:10px\">$i</a>";
                }
            } else {
                echo "<b><a href=\"?per_page=" . $item_per_page ."&SX=".$SX."&idLoai=".$idLoai . "&page=" . $i . "\" style=\"border: 1px solid;font-size: 24px;margin:5px;background: lightblue;;padding:10px\">$i</a></b>";
            }
        }
        if ($tranghientai < $totalpage) {
            echo "<a href=\"?per_page=" . $item_per_page ."&SX=".$SX. "&idLoai=".$idLoai ."&page= " . ($tranghientai + 1) . "\" style=\"border: 1px solid;margin:5px;font-size: 24px;padding:10px\">Trang sau</a>";
        }
        if ($tranghientai < $totalpage - 2) {
            echo "<a href=\"?per_page=" . $item_per_page ."&SX=".$SX. "&idLoai=".$idLoai ."&page= " . $totalpage . "\" style=\"border: 1px solid;margin:5px;font-size: 24px;padding:10px\">Trang cuối</a>";
        }
